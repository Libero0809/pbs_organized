...
char INPUTSETS_FILE[MAX_FILENAME_LEN];
bool USE_EST = false;
...
void arg_parse(int argc, char *argv[]) {
  ...
  memset(INPUTSETS_FILE, 0, MAX_FILENAME_LEN);
  while (true) {
    static struct option long_options[] = {
        ...
        {"inputsetsfilename", optional_argument, nullptr, 'i'},
...
int main(int argc, char *argv[]) {
  ...
  TxTFileReader reader(INPUTSETS_FILE);
    ...
    */
    ...
    if (succeed && correct) {
      ...
      fout << (i + 1) << "," << (BYTES_EACH_CELL * ibltA.hashTableSize()) << ","
           ...
           << t_decoding << "," << succeed << "," << DIFF_SIZE << ","
           << ibltA.hashTableSize() << "," << KEY_SIZE << "," << t << "\n";
    }
  }
}
