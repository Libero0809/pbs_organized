...
STARTER_FILE = "../test-sets/1000000-1000/diff_starter.json"
...
def pbs_experiments(
    ...
    param_file_need_further_process = False
    if best_param_filename_prefix.endswith('.csv'):
        parameter_filename = f"{best_param_filename_prefix}"
    else:
        param_file_need_further_process = True
    ...
    with open(bash_filename, "w") as ofp:
        ...
        for d in [int(s) for s in diffs.split()]:
            ...
            for avg_d in [float(s) for s in avg_d_per_group.split()]:
                if param_file_need_further_process:
                    parameter_filename = f"{best_param_filename_prefix}{max_rounds}_{int(avg_d)}.csv"
                command_str = f"{binary} -n {num_tests} -d {d} -s {starter[str(d)]} -u {union_size} -e {est_filename_} -i {input_set_filename} -r {est_inflation_ratio} -R {max_rounds} -p {parameter_filename} -g {avg_d}"
        ...
        ofp.write(f'mv ./*.txt {result_folder}\n')
...
def gen_bash_for_experiments(
    ...
    """Generate bash file for experiments"""
