...
class search_params:
    ...
    def __init__(self):
        self.params = []
        with open(
            os.path.join(PYBLT_PATH, "param.export.0.995833.2018-07-17.csv")
        ) as f:
            # Skip first line
            line = f.readline()
            line = f.readline()
            while line:
                ...
                self.params.append(
                    (int(items), float(hedge), int(numhashes), int(size), float(p))
                )
                line = f.readline()
    ...
    def bf_num_bytes(self, error_rate, capacity):
        ...
        bits_per_slice = int(
            ceil((capacity * abs(log(error_rate))) / (num_slices * (log(2) ** 2)))
        )
    ...
    def solve_a(self, m, n, x, y):
        ...
        search_space_for_a = np.linspace(
            start=START, stop=denom, endpoint=False, num=NUM
        )
